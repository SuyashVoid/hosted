...
const maxSize = 5;
const minSize = 0;
const baseDownStep = 0.045;
const accDownStep = 0.1;
const connectionThreshold = 100;


let downStep = baseDownStep;
const upStep = 3.64;
const moverRadius = 24;
//let numParticles = 1800;
let numParticles = canvas.width * canvas.height / 800;

let connections = 0;

const connectSkip = 1;
const connectSizeLimit = 0.1;
const connectDistMultiplier = 5;

const dirUpStep = 0.05;
const dirDownStep = 0.006;
...
window.addEventListener('mousemove',
    function(event) {
        // mover.x = this.innerWidth - event.x;
        // mover.y = this.innerHeight - event.y;
        // mover2.x = this.innerWidth - event.x;
        // mover2.y = event.y;
        mover3.x = event.x;
        mover3.y = event.y;
        mover4.x = this.innerWidth - event.x;
        mover4.y = event.y;
        //checkProximity();
    })
...
class Particle {
    ...
    update() {
        ...
        //         //this.connectcolor = connectcolorsRGB[getRandomInt(connectcolorsRGB.length)];
    ...
    for (let i = 0; i < particleArray.length; i += connectSkip) {
        for (let j = 0; j < particleArray.length; j += connectSkip) {
            if (particleArray[i].size > connectSizeLimit && particleArray[j].size > connectSizeLimit) {
                ...
                if (dist < (canvas.width / connectDistMultiplier) * (canvas.height / connectDistMultiplier) && dist > 5200) {
                    let opacity = (particleArray[i].size + particleArray[j].size) / 30 - (dist / 150000);
                    ctx.lineWidth = 1;
                    ctx.strokeStyle = particleArray[i].connectcolor + opacity + ')';
                    if (particleArray[i].connectcolor == connectcolors[4] && dist > 23200 && goldParticles < 100) {
...
function animate() {
    ...
    if (connections > 8000) downStep = accDownStep;
    else downStep = baseDownStep;
    frameIter++;
